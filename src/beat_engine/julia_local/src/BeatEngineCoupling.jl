# Burton-Miller coupling magnitude, and its low-frequency cap.
#
#   eta = s * i,   s = |k| / max(k^2, c)
#
# This file owns the *magnitude* s and the geometry that derives c. The sign of
# eta -- which time convention it belongs to -- lives with `burton_miller_coupling`
# in BeatEnginePhasor.jl: that function composes `propagation_sign()` with the
# magnitude here, so nothing in this file may assume a convention.
#
...
# a sweep sees a jump. On a sphere of radius a the constructed R is sqrt(3) a
# (half the diagonal of the enclosing cube), so the cap engages below
# ka = 1/sqrt(3), not ka = 1; the cited study's radius-a scaling is the same up
# to that constant.
#
...
The real, positive magnitude `s` of `eta = i * s`: `|k| / max(k^2, cap)`.
